...
import com.bilik.records.ClickEvent;
import com.bilik.records.ClickEventSerializationSchema;
import com.bilik.utils.Utils;
...
import static com.bilik.ClickEventCount.WINDOW_SIZE;
import static com.bilik.utils.Utils.log;
...
public class ClickEventGenerator {
	...
	public static void main(String[] args) throws Exception {
		...
		ClickEventSerializationSchema serializationSchema = new ClickEventSerializationSchema(topic);
		while (true) {
			ClickEvent event = clickIterator.next();

			ProducerRecord<byte[], byte[]> record = serializationSchema.serialize(event, null);
	...
	private static Properties createKafkaProperties(final ParameterTool params) {
		String brokers = params.get("bootstrap.servers", "localhost:29092");
