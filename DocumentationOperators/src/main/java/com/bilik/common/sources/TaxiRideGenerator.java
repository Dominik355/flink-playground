...
public class TaxiRideGenerator implements SourceFunction<TaxiRide> {
    ...
    public TaxiRideGenerator(int count) {
        this.count = count;
    }

    @Override
    public void run(SourceFunction.SourceContext<TaxiRide> ctx) throws Exception {
    ...
    public void print() throws Exception {
        ...
        while (running) {
            ...
            // then emit the new START events (out-of-order)
            java.util.Collections.shuffle(startEvents, new Random(id));
            startEvents.iterator().forEachRemaining(r -> System.out.println(r));
